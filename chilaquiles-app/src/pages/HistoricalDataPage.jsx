...
const processDataForChart = (orders, period) => {
  ...
  orders.forEach((order) => {
    ...
    const key = groupByDay ? `${year}-${month}-${day}` : `${year}-${month}-01`;
...
function HistoricalDataPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const dateObj = period === 'day' ? new Date(selectedDate) : undefined;
  ...
  const chartData = useMemo(() => processDataForChart(data.orders, period), [data.orders, period]);
            ...
            <Grid item xs={12} md={4}>
              <Card>
                <CardContent>
                  ...
                  <Typography color="text.secondary">{data.orders.length} órdenes</Typography>
                </CardContent>
              </Card>
            </Grid>
            <Grid item xs={12} md={4}>
              <Card>
                <CardContent>
                  ...
                  <Typography color="text.secondary">{data.purchases.length} compras</Typography>
                </CardContent>
              </Card>
            </Grid>
            <Grid item xs={12} md={4}>
